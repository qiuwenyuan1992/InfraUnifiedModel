...
职责边界：`internal/adapter` 处理鉴权、分页、响应解析与来源字段转换；同步 service 在本轮内存中处理身份映射和关系校验；worker 驱动任务；repository 只写入图节点、已解析关系及 MySQL 同步控制状态。第一阶段已实现 `device_view` 的纵向闭环：领取任务、流式写入设备、状态迁移，并仅在整轮成功后清理旧的无边设备节点；其他实体仍按本章约定逐步接入。
