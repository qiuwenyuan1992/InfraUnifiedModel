...
func TestMySQLRecordsPartialDDLFailureBeforeReleasingLock(t *testing.T) {
	...
	mock.ExpectExec("CREATE TABLE sources").WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec("CREATE TABLE sync_runs").WillReturnError(errors.New("simulated DDL failure"))
...
func TestMySQLRecordsCanceledStatement(t *testing.T) {
	...
	mock.ExpectExec("CREATE TABLE sources").WillReturnError(context.Canceled)
