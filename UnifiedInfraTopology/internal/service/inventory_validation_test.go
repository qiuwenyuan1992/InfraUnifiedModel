...
func TestInventoryRejectsInvalidQueriesAndMissingKey(t *testing.T) {
	...
	for _, tc := range cases {
		if _, err := s.List(ctx, "AliceCase", inventoryScope, tc.resource, tc.parent, tc.q); !errors.Is(err, ErrInventoryInvalid) {
			t.Errorf("%+v: %v", tc, err)
		}
	}
	...
	if _, err := noKey.List(ctx, "AliceCase", inventoryScope, "devices", "", InventoryQuery{}); !errors.Is(err, ErrInventoryNotReady) {
		t.Fatalf("missing cursor key: %v", err)
	}
	if _, err := noKey.GetDevice(ctx, "AliceCase", inventoryScope, inventoryDeviceA, ""); err != nil {
...
func TestInventoryParentAndDuplicateAddressReads(t *testing.T) {
	...
	page, err := s.List(ctx, "AliceCase", inventoryScope, "addresses", inventoryDeviceA, InventoryQuery{AddressFamily: "4"})
	if err != nil {
		t.Fatal(err)
	}
	...
	page, err = s.List(ctx, "AliceCase", inventoryScope, "interfaces", inventoryDeviceA, InventoryQuery{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err = s.List(ctx, "AliceCase", inventoryScope, "addresses", strings.Repeat("c", 32), InventoryQuery{}); !errors.Is(err, ErrInventoryNotFound) {
	...
	if _, err = s.List(ctx, "AliceCase", inventoryScope, "devices", "", InventoryQuery{}); !errors.Is(err, ErrInventoryNotReady) {
...
func TestInventoryRunValidationRollbackAndPrivateRun(t *testing.T) {
	...
	for _, req := range cases {
		if _, err := s.Enqueue(ctx, "AliceCase", inventoryScope, "invalid", req); !errors.Is(err, ErrInventoryInvalid) {
			t.Fatalf("invalid request: %v", err)
		}
	}
	req := EnqueueInventoryRun{Mode: "full", SourceIDs: []string{inventorySource}}
	for _, key := range []string{"", "bad\nkey", strings.Repeat("x", 129)} {
		if _, err := s.Enqueue(ctx, "AliceCase", inventoryScope, key, req); !errors.Is(err, ErrInventoryInvalid) {
	...
	if _, err := s.Enqueue(ctx, "AliceCase", inventoryScope, "rollback", req); err == nil {
	...
	inventoryCreate(t, db, &model.SyncRun{ID: inventoryDeviceA, ScopeID: inventoryOther, Status: "queued", IdempotencyKey: "private", CreatedAt: time.Now().UTC()})
	if _, err := s.GetRun(ctx, "AliceCase", inventoryScope, inventoryDeviceA); !errors.Is(err, ErrInventoryNotFound) {
		t.Fatalf("private run: %v", err)
	}
	if _, accepted, err := s.CancelRun(ctx, "AliceCase", inventoryScope, inventoryDeviceA); accepted || !errors.Is(err, ErrInventoryNotFound) {
		t.Fatalf("private cancellation: %v", err)
	}
}

func TestInventoryCancellationFailureIsNotAccepted(t *testing.T) {
	s, db, _ := inventoryFixture(t)
	inventoryCreate(t, db, &model.SyncRun{ID: inventoryDeviceA, ScopeID: inventoryScope, Status: "queued", IdempotencyKey: "cancel-failure"})
...
func TestInventoryRunPaginationAndPendingCancellation(t *testing.T) {
	...
	for _, id := range []string{inventoryDeviceA, inventoryDeviceB} {
		inventoryCreate(t, db, &model.SyncRun{ID: id, ScopeID: inventoryScope, Status: "running", IdempotencyKey: id, CreatedAt: now})
	}
	first, err := s.List(ctx, "AliceCase", inventoryScope, "sync-runs", "", InventoryQuery{Limit: 1, Status: "running"})
	if err != nil {
		t.Fatal(err)
	}
	...
	next, err := s.List(ctx, "AliceCase", inventoryScope, "sync-runs", "", InventoryQuery{Limit: 1, Status: "running", Cursor: *first.NextCursor})
	if err != nil {
		t.Fatal(err)
	}
	...
	run, accepted, err := s.CancelRun(ctx, "AliceCase", inventoryScope, inventoryDeviceA)
	...
	again, accepted, err := s.CancelRun(ctx, "AliceCase", inventoryScope, inventoryDeviceA)
